...
/* المزامنة اللحظية الجديدة: كل صنف بيتبعت لوحده لمساره الخاص، مش المصفوفة كاملة.
   ...
   dirtyItemCodes: أكواد الأصناف اللي عندها تعديل معلّق لسه ما اترفعش (لإعادة المحاولة عند رجوع النت) */
...
function patchSingleRow(item){
  ...
  const noteBtn = tr.querySelector('[data-note]'); if (noteBtn) noteBtn.classList.toggle('has-note', !!item.note);
  refreshRow(tr, item);
}
...
function updateStats(){
  ...
  const dtd = $('dashTopDiff');
  if (dtd) {
    if (!total) { dtd.textContent = '—'; }
    else {
      const top = inventoryData.reduce((a, i) => Math.abs(i.difference) > Math.abs(a.difference) ? i : a, inventoryData[0]);
      dtd.textContent = top.difference === 0 ? 'مفيش فروق' : (top.difference > 0 ? '+' : '') + fmtQ(top.difference) + ' (' + top.code + ')';
    }
  }
}
...
function setSyncUI(state, txt){
  ...
  const dd = $('dashLastSync'); if (dd) dd.textContent = pad2(d.getHours()) + ':' + pad2(d.getMinutes());
}
...
async function connectFirebase(silent, retryCount){
  ...
  try {
    ...
    try {
      db.ref('.info/connected').on('value', s => {
        ...
        if (s.val() === true) {
          ...
          /* النت رجع؟ ارفع أي تعديلات محلية معلّقة — كل صنف لوحده، مش استبدال شامل */
          if (pendingOfflinePush && seenData) {
            pendingOfflinePush = false;
            flushDirtyItems();
            if (wasOff && bootDone) toast('✅ النت رجع — اترفعت تعديلاتك المعلّقة', 'success');
          }
    ...
    /* ⚖️ ممنوع نستبدل بيانات السيرفر بنسخة محلية قديمة — نرفع بس التعديلات المعلّقة فعلياً، صنف صنف */
    if (pendingOfflinePush && seenData) { pendingOfflinePush = false; flushDirtyItems(); }
...
function pushFullReplace(items){
  if (!syncOn || !db) { pendingOfflinePush = true; toast('مفيش اتصال — هيترفع لما النت يرجع', 'warning'); return Promise.resolve(false); }
  const obj = {};
  items.forEach(it => { obj[itemKey(it.code)] = it; });
  return db.ref(fbPath() + '/items').set(obj).then(() => { flashDot(); accessDenied = false; updateOfflineBar(); return true; }).catch(err => {
    lastSyncErr = err && err.message ? err.message : String(err);
...
function pushMergeUpdate(changedItems, removedCodes){
  ...
  return db.ref(fbPath() + '/items').update(patch).then(() => { flashDot(); accessDenied = false; updateOfflineBar(); return true; }).catch(err => {
    lastSyncErr = err && err.message ? err.message : String(err);
...
async function openNote(serial){
  ...
  const v = await new Promise(res => {
    const m = showModal('ملاحظات — ' + item.code,
      '<div class="fld"><textarea id="_dlgArea" style="min-height:110px;direction:rtl;text-align:right;font-size:.85rem"></textarea></div>',
      [
  ...
  if (!serverOk) {
    try {
      ...
      if (cfg && cfg.databaseURL) {
        ...
        const r1 = await fetch(base + '/' + fbPath() + '/items.json', { method: 'DELETE' });
  ...
  if (!serverOk) {
    ...
    try {
      ...
      if (cfg && cfg.databaseURL) {
        ...
        serverOk = r1.ok;
        if (!serverOk) failReason = 'HTTP ' + r1.status;
      } else if (!failReason) {
        failReason = 'مفيش اتصال بقاعدة البيانات دلوقتي';
      }
    } catch(e){ failReason = e && e.message ? e.message : String(e); }
...
async function openSettings(){
  ...
  const body =
    ...
    '<div style="display:flex;gap:.5rem;margin-bottom:.7rem;flex-wrap:wrap"><button class="mbtn primary" id="refreshLogBtn">🔄 تحديث السجل</button><button class="mbtn ghost" id="clearLogBtn">🗑️ مسح سجل الجرد</button><button class="mbtn ghost" id="exportLogBtn">⬇️ تصدير السجل</button></div>' +
...
window.addEventListener('load', () => {
  ...
  window.addEventListener('online', () => {
    ...
    else if (pendingOfflinePush) { pendingOfflinePush = false; flushDirtyItems(); }
    updateOfflineBar();
  });
  ...
  setInterval(() => {
    if (navigator && navigator.onLine === false) return;
    if (syncOn && pendingOfflinePush) { pendingOfflinePush = false; flushDirtyItems(); }

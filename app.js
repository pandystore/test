...
async function loadCountLog(){
  ...
  try {
    ...
    let html = '<table id="countLogTable" style="width:100%;font-size:.75rem;border-collapse:collapse"><thead><tr style="background:#1e293b;color:#fff"><th style="padding:6px">الوقت</th><th style="padding:6px">المستخدم</th><th style="padding:6px">اسم الصنف</th><th style="padding:6px">الكمية</th></tr></thead><tbody>';
    items.forEach(ev=>{
      const d = ev.ts ? new Date(ev.ts) : new Date();
      const time = d.toLocaleString('ar-EG', { hour12:false });
      html += '<tr style="border-bottom:1px dashed #e2e8f0"><td style="padding:5px;direction:ltr">' + time + '</td><td style="padding:5px">👤 ' + esc(ev.by||'') + '</td><td style="padding:5px">' + esc(ev.name||'') + '</td><td style="padding:5px;font-weight:800;text-align:center">' + fmtQ(ev.qty||0) + '</td></tr>';
    });
    html += '</tbody></table>';
    box.innerHTML = html;
...
function exportCountLog(){
  ...
  db.ref(fbPath() + '/notifs').orderByKey().limitToLast(200).get().then(snap=>{
    ...
    let csv = 'الوقت,المستخدم,اسم الصنف,الكمية\n';
    items.forEach(ev=>{
      const d = ev.ts ? new Date(ev.ts).toISOString() : '';
      csv += '"' + d + '","' + (ev.by||'') + '","' + (ev.name||'').replace(/"/g,'""') + '",' + (ev.qty||0) + '\n';
    });
...
function exportCountLogPDF(){
  ...
  db.ref(fbPath() + '/notifs').orderByKey().limitToLast(200).get().then(snap=>{
    ...
    items.forEach(ev=>{
      const d = ev.ts ? new Date(ev.ts) : new Date();
      const time = d.toLocaleString('ar-EG', { hour12:false });
      rows += '<tr><td>' + esc(time) + '</td><td>' + esc(ev.by||'') + '</td><td>' + esc(ev.name||'') + '</td><td>' + fmtQ(ev.qty||0) + '</td></tr>';
    });
    const html = '<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>سجل الجرد</title><style>' +
      ...
      '<table><thead><tr><th>الوقت</th><th>المستخدم</th><th>اسم الصنف</th><th>الكمية</th></tr></thead><tbody>' + rows + '</tbody></table>' +
      '</body></html>';
